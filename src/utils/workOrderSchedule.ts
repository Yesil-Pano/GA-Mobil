...
/** Yalnızca iş tipi — kategori "Arıza Bildirimi" periyodik bakımı arıza saymaz. */
export function isArızaWorkOrder(order: Pick<WorkOrder, 'type' | 'category'>): boolean {
  const type = (order.type ?? '').toLocaleLowerCase('tr-TR');
  return type.includes('arıza') || type.includes('ariza');
}

export function isTrueArızaWorkOrder(
  order: Pick<WorkOrder, 'type' | 'category' | 'isPeriodic' | 'parentWorkOrderId'>,
): boolean {
  return isArızaWorkOrder(order) && !order.isPeriodic && !order.parentWorkOrderId;
}

export function isPeriodicContext(
  order: Pick<WorkOrder, 'isPeriodic' | 'parentWorkOrderId'>,
): boolean {
  return !!order.isPeriodic || !!order.parentWorkOrderId;
}

/** StartDate gelecek Türkiye takvim ayında mı? */
export function isFutureTurkeyPeriod(
  ...
  if (orderYm.year !== nowYm.year) return orderYm.year > nowYm.year;
  return orderYm.month > nowYm.month;
}
 ...
 * - Gerçek arıza (periyodik değil): her zaman
...
export function isVisibleOnMobileList(
  order: Pick<WorkOrder, 'status' | 'startDate' | 'endDate' | 'type' | 'category' | 'isPeriodic' | 'parentWorkOrderId'>,
  now: Date = new Date(),
): boolean {
  if (isWorkOrderFinished(order.status)) return true;
  if (isTrueArızaWorkOrder(order)) return true;
  if (isPeriodicContext(order)) return !isFutureTurkeyPeriod(order, now);
  return isInCurrentTurkeyMonth(order, now);
}

/** Periyodik dönem henüz gelmediyse saha kapatamaz. */
export function canFieldCloseWorkOrder(
  order: Pick<WorkOrder, 'type' | 'category' | 'isPeriodic' | 'parentWorkOrderId' | 'startDate'>,
  now: Date = new Date(),
): boolean {
  if (isTrueArızaWorkOrder(order)) return true;
  if (!isPeriodicContext(order)) return true;
  return !isFutureTurkeyPeriod(order, now);
}
